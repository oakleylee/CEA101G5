...
<html>
<head>
<title>所有權限分類 - listAllEmpAuth.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>所有員工權限 - listAllEmpAuth.jsp</h3>
...
<table>
	...
	<c:forEach var="eaVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${eaVO.auth_no}</td>
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/empauth/ea.do" style="margin-bottom: 0px;">
			     <input type="submit" value="修改">
			     <input type="hidden" name="emp_id"  value="${eaVO.emp_id}">
			     <input type="hidden" name="auth_no"  value="${eaVO.auth_no}">
